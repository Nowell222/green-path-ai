...
import { 
  LayoutDashboard, Users, Truck, Map, FileText, BarChart3, Bell, Settings, LogOut, Leaf, ChevronLeft, ChevronRight, Search, Package
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { Input } from '@/components/ui/input';

interface HaulingAdminLayoutProps { children: ReactNode; }

const navItems = [
  { path: '/hauling', label: 'Dashboard', icon: LayoutDashboard },
  { path: '/hauling/ecoaides', label: 'Eco-Aides', icon: Users },
  { path: '/hauling/fleet', label: 'Fleet Management', icon: Truck },
  { path: '/hauling/routes', label: 'Routes', icon: Map },
  { path: '/hauling/requests', label: 'Hauling Requests', icon: Package },
  { path: '/hauling/analytics', label: 'Analytics', icon: BarChart3 },
  { path: '/hauling/notifications', label: 'Notifications', icon: Bell },
];

export default function HaulingAdminLayout({ children }: HaulingAdminLayoutProps) {
    ...
    <div className="min-h-screen bg-background flex">
      <aside className={cn(
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
          {!collapsed && (
            <Link to="/hauling" className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-sidebar-foreground/60">Hauling Admin</p>
              </div>
            </Link>
          )}
          <Button variant="ghost" size="icon" className="text-sidebar-foreground hover:bg-sidebar-accent h-8 w-8" onClick={() => setCollapsed(!collapsed)}>
        ...
        <div className="p-2 border-t border-sidebar-border">
          <Link to="/hauling/settings" className={cn(
        ...
        <header className="sticky top-0 z-40 h-16 bg-card border-b border-border flex items-center justify-between px-6">
          <div className="flex items-center gap-4 flex-1 max-w-md">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search eco-aides, requests..." className="pl-9 bg-background" />
            </div>
          </div>
